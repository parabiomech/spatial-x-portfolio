...
import {ATTACK_Z,BALL_HEIGHT,DEFENDER_Z,LISTENER_HEIGHT,targetWorldPosition} from './spatial.js?v=45';
...
export class CourtView {
  constructor(host) {
    ...
    this.ball.add(new THREE.Mesh(new THREE.SphereGeometry(.21,24,18),new THREE.MeshBasicMaterial({color:0x1475ed,transparent:true,depthTest:false,depthWrite:false})));
    this.ring=new THREE.Mesh(new THREE.RingGeometry(.18,.27,48),new THREE.MeshBasicMaterial({color:0x15c9ff,transparent:true,opacity:0,side:THREE.DoubleSide,depthTest:false,depthWrite:false}));
  ...
  async loadBall(){try{const loader=new THREE.TextureLoader();const [texture,normal,model]=await Promise.all([loader.loadAsync('./assets/ball/ballO6.jpg'),loader.loadAsync('./assets/ball/ballO6_normal.jpg'),new FBXLoader().loadAsync('./assets/ball/ballO6.fbx')]);texture.colorSpace=THREE.SRGBColorSpace;const bounds=new THREE.Box3().setFromObject(model),size=bounds.getSize(new THREE.Vector3()),center=bounds.getCenter(new THREE.Vector3()),scale=.42/Math.max(size.x,size.y,size.z);model.position.sub(center).multiplyScalar(scale);model.scale.setScalar(scale);model.traverse(child=>{if(child.isMesh){const hasUV=!!child.geometry?.getAttribute('uv');child.material=new THREE.MeshStandardMaterial({color:hasUV?0xffffff:0x2879d5,map:hasUV?texture:null,normalMap:hasUV?normal:null,emissive:hasUV?0xffffff:0x164985,emissiveMap:hasUV?texture:null,emissiveIntensity:.7,roughness:.82,metalness:0,side:THREE.DoubleSide,transparent:true,depthTest:false,depthWrite:false});child.renderOrder=999;}});this.ball.clear();this.ball.add(model);}catch(e){console.warn('FBX ball fallback',e);}}
  async loadScan(){
    try {
      ...
      this.hall.rotateOnWorldAxis(new THREE.Vector3(0,1,0),THREE.MathUtils.degToRad(2.5));
