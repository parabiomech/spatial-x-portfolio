...
function settings(){return{version:'0.6.0',participant:$('participant').value.trim()||'anonymous',experienceYears:$('experience').value===''?null:Number($('experience').value),device:$('device').value.trim(),dataset:$('dataset').value,sound:$('sound').value,mode:$('mode').value,inputMode:$('input-mode').value,repeats:Number($('repeats').value),leadMs:Number($('lead').value),length:$('length').value,seed:$('seed').value||'spatial-01',visualPhysics:{speed:Number($('throw-speed').value),angle:Number($('throw-angle').value),height:Number($('throw-height').value)},gainDb:$('dataset').value==='trajectory'||$('sound').value==='pinknoise'?0:5,audioMode:'recorded stereo unchanged; pilot pink noise uses browser HRTF',responseMode:'first input, first valid selection, final confirmation; estimated scheduled audio onset/offset'};}
function update(){if(!bank)return;const set=$('dataset').value,kinds=new Set(bank.clips.filter(c=>c.dataset===set&&!c.reference).map(c=>c.kind));for(const option of $('sound').options)option.disabled=option.value==='both'?kinds.size<2:!kinds.has(option.value);if($('sound').selectedOptions[0].disabled)$('sound').value=kinds.has('impact')?'impact':([...kinds][0]||'impact');const quick=$('stimulus-shortcuts');if(quick){quick.hidden=set==='trajectory';quick.replaceChildren();for(const [v,label] of [['pinknoise','핑크노이즈'],['impact','임팩트'],['bell','방울소리'],['shake','공 흔들기']]){const b=document.createElement('button'),option=[...$('sound').options].find(o=>o.value===v);b.type='button';b.textContent=label;b.disabled=!option||option.disabled;b.setAttribute('aria-pressed',String($('sound').value===v));b.onclick=()=>{$('sound').value=v;update();};quick.append(b);}}$('input-mode').options[1].disabled=set!=='ring'&&set!=='ring8';if(set!=='ring'&&set!=='ring8')$('input-mode').value='buttons';const dynamic=set==='trajectory';$('mode').options[0].textContent=dynamic?'연습 · 4투구 × 3단계 / 단계마다 판단':'연습 · 4회 / 정답 공개';$('sound').closest('label').hidden=dynamic;$('input-mode').closest('label').hidden=dynamic;$('task-hint').textContent=dynamic?'연습은 같은 투구를 1/3, 2/3, 3/3까지 듣고 각각 응답합니다. 평가는 차단 길이를 섞습니다.':set==='court'?'청취 위치 C0 · 상대편 9m 골대 방향': '정면 0° 기준 · 오른쪽으로 '+(set==='ring8'?45:30)+'°씩 선택';try{const c=settings(),plan=makePlan(bank.clips,c);$('count').textContent=c.dataset==='trajectory'?`${plan.length}시행 · 음원 총 길이의 1/3·2/3·전체 · 단계마다 1회 응답`:`${plan.length}회 · ${c.mode==='practice'?'짧은 연습 / 피드백 제공':'평가 / 조건별 '+c.repeats+'회 반복'} · ${c.dataset==='ring'?'12방향':c.dataset==='ring8'?'8방향 · 합성 예비 자극':c.dataset==='court'?'15지점':'3단계 시간차단'}`;$('start').disabled=!bank.ready;}catch(e){$('count').textContent=e.message;$('start').disabled=true;}view?.setDataset(set,targets(set));view?.listenerView();}
...
try{const {CourtView}=await import('./scene.js?v=41');view=new CourtView($('setup-scene'));if(bank)view.setDataset($('dataset').value,targets($('dataset').value));$('scene-status').textContent='경기장 공간 · 소리 위치는 공으로 표시';}catch(e){$('scene-status').textContent='이 기기에서 3D 화면을 열지 못했습니다. 음원과 버튼 테스트는 계속 사용할 수 있습니다.';}
...
$('view-listener').textContent='청취자 시점';$('view-scan').textContent='SOG 공간 표시 전환';$('view-scan').setAttribute('aria-pressed','true');
...
$('familiar-set').onchange=()=>{renderFamiliar();const set=$('familiar-set').value;view?.setDataset(set,targets(set));view?.listenerView();};
