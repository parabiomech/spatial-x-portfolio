...
try{const {CourtView}=await import('./scene.js?v=45');view=new CourtView($('setup-scene'));if(bank)view.setDataset($('dataset').value,targets($('dataset').value));$('scene-status').textContent='경기장 공간 · 소리 위치는 공으로 표시';}catch(e){$('scene-status').textContent='이 기기에서 3D 화면을 열지 못했습니다. 음원과 버튼 테스트는 계속 사용할 수 있습니다.';}
