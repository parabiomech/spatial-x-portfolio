...
export function chartSvg(rows,key,dataset='ring'){
 const temporal=key==='correct'&&dataset==='trajectory',cuts=[1,2,3];
 const vals=temporal?cuts.map(c=>{const group=rows.filter(r=>r.stageNumber===c);return group.length?mean(group.map(r=>+r.correct)):0}):rows.map(r=>key==='correct'?+r.correct:Number(r.firstChoiceFromOffsetMs));
 ...
 const dots=pts.map((p,i)=>{const label=temporal?`${cuts[i]}/3`: `시행 ${i+1}`;return`<circle cx="${p[0]}" cy="${p[1]}" r="${temporal?6:4}" fill="${key==='correct'?(vals[i]>.5?'#72e3bf':'#ed7666'):'#ffbd72'}"><title>${label}: ${key==='correct'?Math.round(vals[i]*100)+'%':Math.round(vals[i])+' ms'}</title></circle>${temporal?`<text x="${p[0]}" y="${H-18}" text-anchor="middle" fill="#a6b8d0" font-size="12">${label}</text>`:''}`}).join('');
